...
export class PokemonService {
  ...
  private handleExceptions(error: any){
    if (error.code === 11000) {
      throw new BadRequestException(`Pokemon existente en base de datos ${JSON.stringify(error.keyValue)}`);
    }
    throw new InternalServerErrorException(`No se pudo crear el pokemon - verifica los logs`);
  }
}
